package common.nodes.nodeImplementations;

import common.distributedAlgos.ColeVishkin;
import common.globals.BaseCustomGlobal;
import common.nodes.messages.ForestJoinMessage;
import common.nodes.messages.ParentForestColorMessage;
...
public class BaseDistNode extends Node {
    ...
    public void sendColorToChildren(int forest, int colorBitInt) {
        ...
        for (BaseDistNode child : forestChildren) {
            send(new ParentForestColorMessage(forest, colorBitInt), child);
        }
    }
    ...
    public void handleMessages(Inbox inbox) {
        ...
        while(inbox.hasNext()) {
            ...
            if (msg instanceof ParentForestColorMessage) {
                ParentForestColorMessage m = (ParentForestColorMessage)msg;
                updateParentColor(m.forest, m.newColor);
            } else if (msg instanceof ForestJoinMessage) {
                ForestJoinMessage m = (ForestJoinMessage)msg;
    ...
    protected void forestDecomposition() {
        ...
        for (int forest = 1; forest <= BaseCustomGlobal.maxDegree; forest++) {
	        ...
	        if (parent != null) {
	        	send(new ForestJoinMessage(forest), parent);
	        }
        }
    }
